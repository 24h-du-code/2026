"""
modules/games/lava_key_door_maze.py — Lava + Key & Door Maze agent (Game #9)

Two-phase BFS: player → key → exit.
Avoids walls (#), lava (L); door (D) blocked until key collected.
Stops immediately if no safe path exists (avoids losing points).
    ...
    while status == "continue":
        ...
        if not has_key:
            ...
            if kpos:
                ...
                # Key already collected but state not yet updated — head to exit
...
def run():
    ...
    while True:
        ...
        net = results["win"] * 10 - results["lose"]
